  ...
  return (
    <div>
      ...
      <div className="relative">
        {icon && (
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#3b4f66]">{icon}</span>
        )}
        <input
          id={id}
    ...
    <div className="min-h-screen bg-[#070d18] flex">
      ...
      <div className="hidden lg:flex lg:w-[46%] relative overflow-hidden bg-[#080e1a] border-r border-[#1e2d4a]/50 flex-col justify-between p-12">
        ...
        <div className="relative z-10 space-y-5">
          <p className="text-[#4a5d75] text-sm uppercase tracking-widest font-medium mb-6">
            Trusted by developers
          </p>
          {[
            { value: "2,400+", label: "Active developers" },
            { value: "18,000+", label: "Roadmaps generated" },
            { value: "94%",    label: "Landed their first job" },
          ].map((stat) => (
            <div key={stat.label} className="flex items-center gap-4">
              <div className="text-2xl font-black text-white w-24">{stat.value}</div>
              <div className="text-[#4a5d75] text-sm">{stat.label}</div>
            </div>
          ))}
          ...
          <div className="pt-8">
            <div className="p-5 rounded-2xl bg-[#0a1628]/80 border border-[#1e3a5f]/60">
              <p className="text-[#8899aa] text-sm leading-relaxed italic mb-4">
                "I was stuck in tutorial hell for 2 years. RoadmapAI's structured path helped me land a React role in 4 months."
              </p>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#06b6d4] to-[#3b82f6] flex items-center justify-center text-white text-xs font-bold">
                  PS
                </div>
                <div>
                  <p className="text-white text-xs font-semibold">Priya S.</p>
                  <p className="text-[#4a5d75] text-xs">Frontend Dev @ Swiggy</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="relative flex items-center gap-3 mb-6">
            ...
            <span className="text-[#2e3f55] text-xs">or continue with email</span>
            <div className="flex-1 h-px bg-[#1e2d4a]" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label htmlFor="password" className="text-xs font-medium text-[#8899aa] uppercase tracking-wider">
                  Password
                </label>
                <a href="#" className="text-xs text-[#3b82f6] hover:text-[#60a5fa] transition-colors">
                  Forgot password?
                </a>
              </div>
              <InputField
          ...
          <p className="text-[#2e3f55] text-xs text-center mt-6">
            New here?{" "}
            <Link to="/register" className="text-[#4a5d75] hover:text-[#8899aa] transition-colors">
